...
var require_RosLib = __commonJS({
  "node_modules/roslib/src/RosLib.js"(exports, module) {
    ...
    var assign = require_object_assign();
    assign(ROSLIB2, require_core());
    assign(ROSLIB2, require_actionlib());
    assign(ROSLIB2, require_math());
    assign(ROSLIB2, require_tf());
    assign(ROSLIB2, require_urdf());
    module.exports = ROSLIB2;
  }
});

// src/index.ts
var import_roslib = __toESM(require_RosLib(), 1);
...
var createPanelInstance = (context) => {
  ...
  const topicTypes = /* @__PURE__ */ new Map();
  ...
  const renderFieldControls = () => {
    ...
    if (draftFieldPaths.length === 0) {
      const automatic2 = document.createElement("span");
      automatic2.className = "rb-timeseries__helper";
      automatic2.textContent = "Auto-detect is enabled";
      selected.append(automatic2);
    } else {
    ...
    picker.value = "";
    helper.textContent = discoveredFields.length ? `${discoveredFields.length} numeric field${discoveredFields.length === 1 ? "" : "s"} available from the latest message.` : "Apply with auto-detect to find numeric data in the first message.";
  };
  ...
  const configureSubscription = () => {
    ...
    topic = new import_roslib.default.Topic({
      ros: context.ros,
      name: config.topic,
  ...
  return {
    mount(container) {
      ...
      query('[data-field="topic"]').addEventListener(
        "change",
        (event) => {
